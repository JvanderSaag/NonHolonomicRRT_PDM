...
def RRT(N_iter, start, goal, area):
    ...
    for n in range(N_iter):
        ...
        if not Scenario.collision_free(shapely.geometry.Point((posx, posy))):
            continue

        nearest_ver, nearest_ver_id = closest_ver((posx, posy))

        if nearest_ver is not None:
            new_id = tree.add_ver((posx, posy))
            tree.add_edge(nearest_ver_id, new_id)
            plt.plot(shapely.geometry.LineString([(tree.ver_id.values()).index(nearest_ver_id), (tree.ver_id.values()).index(new_id)]))
