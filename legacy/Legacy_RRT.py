...
def RRT(N_iter, scenario, step_size=float('inf'), dist_tolerance=1): # RRT using TreeNodes
    ...
    for n in tqdm(range(N_iter)): # Max N_iter iterations
        ...
        sampled_Node = TreeNode(sampled_point, sample_orientation)
...
def find_closest_Node(scenario, start_Node, new_Node, min_length=float('inf')): # Find closest Node in Tree
    ...
    # CONNECTOR FUNCTION #
    maxc = 0.1
    sx, sy, syaw = start_Node.point.x, start_Node.point.y , start_Node.yaw
    gx, gy, gyaw = new_Node.point.x, new_Node.point.y, new_Node.yaw
    connect_line_list = reeds_shepp_path_planning(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=0.2)
    # Create a line that connects to the new Node

    # COLLISION CHECK #
    if connect_line_list is None:
